  ...
  return (
    ...
    >
      ...
      <div className="relative max-w-7xl mx-auto px-4 z-10">
        ...
        <motion.div
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-6"
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '-40px' }}
        ...
        >
          {destinations.map((dest, index) => (
            <motion.div
              ...
              className={`bg-white/6 border border-white/10 rounded-2xl p-3 lg:p-8 cursor-pointer active:brightness-95 group flex flex-col h-full card-hover card-hover-navy relative overflow-hidden ${index >= 3 ? 'hidden sm:block' : ''}`}
            >
              <Link href="/destinations" className="absolute inset-0 z-10" />
              <div
                className="w-10 h-10 lg:w-16 lg:h-16 rounded-lg lg:rounded-2xl overflow-hidden flex-shrink-0 shadow-lg mx-auto mb-3 lg:mb-5 group-hover:scale-110 transition-transform duration-300 country-badge"
              >
                <Image
              ...
              <h3 className="text-white font-heading font-bold text-sm lg:text-xl text-center group-hover:text-gold transition-colors relative z-20 mb-0.5">
                {dest.country}
              </h3>
              <div className="w-8 h-px bg-gold/40 mx-auto my-2 lg:my-4 relative z-20"></div>
              ...
              <div
                className="mt-2 lg:mt-6 w-full border border-white/20 text-white/70 text-[10px] lg:text-sm py-1.5 lg:py-2.5 rounded-lg lg:rounded-xl group-hover:bg-gold group-hover:text-navy transition-all duration-300 text-center flex items-center justify-center relative z-20 font-medium"
              >
                View Programs
              </div>
            </motion.div>
